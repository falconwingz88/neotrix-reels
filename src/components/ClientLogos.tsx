export const ClientLogos = () => {
  ...
  return (
    <div className="bg-white/5 backdrop-blur-xl rounded-3xl border border-white/20 p-8 shadow-2xl overflow-hidden">
      <div className="mb-8 text-center">
        <h2 className="text-3xl font-bold text-white mb-2">Trusted by Industry Leaders</h2>
        <p className="text-white/70">Proud to collaborate with amazing brands worldwide</p>
      </div>
          ...
          >
            {rowLogos.map((logo, logoIndex) => (
              <div
                ...
                className="flex-shrink-0 w-32 h-16 flex items-center justify-center bg-white/10 rounded-xl border border-white/10 hover:bg-white/20 transition-all duration-300"
              >
                <img
                  src={logo.url}
                  alt={logo.name}
                  className="max-w-24 max-h-12 object-contain filter brightness-0 invert opacity-70 hover:opacity-100 transition-opacity duration-300"
                  onError={(e) => {
